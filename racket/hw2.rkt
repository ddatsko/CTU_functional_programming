...
(define (process-application application global-env local-env)
  (match application
    ((cons 'circle _) (apply circle (map (curry process-arg global-env local-env) (cdr application))))
    ((cons 'rect _) (apply rect (map (curry process-arg global-env local-env) (cdr application))))
    ((cons 'line _) (apply line (map (curry process-arg global-env local-env) (cdr application))))
...
(define (process-function-application application global-env local-env)
  ...
  (let* ([func (get-function global-env local-env (car application))]
         [new-local-env (make-local-env global-env local-env (function-vars func) (cdr application))])
